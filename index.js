...
app.post('/api/auth/register', (req, res) => {
  ...
  if (!inviteCode || String(inviteCode).trim() === '') {
    return res.status(400).json({ error: 'Invite code mandatory' });
  }
...
app.post('/api/wallet/deposit', (req, res) => {
  ...
  if (!numAmount || numAmount <= 0 || !txnId) return res.status(400).json({ error: 'Invalid deposit' });
  ...
  res.json({ success: true, message: 'Deposit requested', depositId: record.id });
});
...
app.post('/api/wallet/claim-loss-bonus', (req, res) => {
  ...
  if (user.balance > 0) return res.status(400).json({ error: 'Balance must be 0' });
  if (user.lastRealDeposit <= 0) return res.status(400).json({ error: 'No deposit history' });
  if (user.lossBonusClaimed) return res.status(400).json({ error: 'Already claimed' });

  const bonusAmount = user.lastRealDeposit * 0.50;
  user.balance += bonusAmount;
  user.lossBonusClaimed = true;

  res.json({ success: true, message: `50% Bonus added: ₹${bonusAmount}`, balance: user.balance });
});
...
app.post('/api/admin/action-deposit', (req, res) => {
  ...
  if (!deposit || deposit.status !== 'pending') {
    return res.status(400).json({ error: 'Deposit request not found' });
  }
...
app.post('/api/admin/set-result', (req, res) => {
  ...
  game.forcedNext = num;
  res.json({ success: true, message: `Next result for ${mode} set to ${num}` });
});

app.post('/api/admin/create-giftcode', (req, res) => {
  const { code, amount } = req.body;
  if (!code || !amount || Number(amount) <= 0) return res.status(400).json({ error: 'Invalid code/amount' });

  db.giftCodes[code] = { amount: Number(amount), claimedBy: [] };
  res.json({ success: true, message: 'Gift code created', code, amount: Number(amount) });
});

// JSON Catch-All Route (Solves HTML <!DOCTYPE... error permanently)
app.use((req, res) => {
  res.status(404).json({ error: `Route ${req.originalUrl} does not exist on server.` });
});
